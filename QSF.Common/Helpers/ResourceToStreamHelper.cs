...
namespace Telerik.Windows.Controls.QuickStart.Common.Helpers
{
	public class ResourceToStreamHelper
	{
		...
		{
#if WPF
            string executingAssembly = uriSource.OriginalString.Split(new char[] { ';', '/' }, StringSplitOptions.RemoveEmptyEntries)[0];

            var targetAssembly = AppDomain.CurrentDomain.GetAssemblies().FirstOrDefault(a => executingAssembly.Equals(a.GetName().Name));
